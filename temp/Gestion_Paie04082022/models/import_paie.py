...
class HrReportBanque(models.TransientModel):
    ...
    def remplissages(self):
        ...
        for vals in self:
            vals.env.cr.execute(
                """SELECT (P.id) AS id, (P.name) AS nom, (E.x_net_payer) AS net,(E.num_banque) as numb, (EM.name) as emploi,(FC.name) as fonction, (E.name) AS employe FROM hr_employee E ,hr_fonctionss FC, hr_emploi EM, res_bank R, hr_payslip P WHERE E.id = P.employee_id and FC.id = E.x_fonction_id and E.x_emploi_id = EM.id and E.x_banque_id = R.id and P.date_from >= %s  and P.date_to <= %s  and R.id = %s and E.company_id = %s and P.state = 'done' ORDER BY employe ASC""",
                (ddbut, ddfin, banq_id, x_struct_id))
            rows = vals.env.cr.dictfetchall()
            result = []
            ...
            text = ''
            for line in rows:
                result.append((0, 0, {'name': line['employe'], 'numb': line['numb'], 'x_emploi': line['emploi'],
                                      'x_fonction': line['fonction'], 'x_sal_net': line['net']}))
                self.x_mnts += line['net']
            self.x_line_ids = result
            text += num2words(vals.x_mnts, lang='fr')
...
class HrReportAvoirs(models.TransientModel):
    ...
    def avoir(self):
        if self.x_type_employe_id and self.x_elt_sal_id and self.x_date_debut and self.x_date_fin:
            ...
            for vals in self:
                vals.env.cr.execute(
                    """SELECT (L.total) AS Total,(E.name) AS employe,(E.matricule) as mat,(E.matricule_genere) as mat_c FROM hr_payslip_line L,hr_payslip LP, hr_employee E where L.slip_id = LP.id and E.id = L.employee_id and LP.struct_id =%s and L.salary_rule_id = %s and LP.date_from >= %s  and LP.date_to <= %s and LP.company_id = %s and LP.state = 'done' AND E.active = 'True'""",
                    (emp_type, elt_sal, ddbut, ddfin, x_struct_id))
                rows = vals.env.cr.dictfetchall()
                result = []
                ...
                for line in rows:
                    result.append((0, 0,
                                   {'name': line['employe'], 'x_matricule': line['mat'], 'x_matricule_c': line['mat_c'],
                                    'x_mnt': line['total']}))
...
class HrLivrePaie(models.TransientModel):
    ...
    def action_rech(self):
        if self.name:
            ...
            ddfin = str(self.x_date_fin.strftime("%Y-%m-%d"))
            for vals in self:
                if x_ty == 'FCT_DETACH' or x_ty == 'FCT_MD' or x_ty == 'Fonctionnaire Detaché' or x_ty == 'Fonctionnaire Mis à Disposition' or x_ty == 'FONCTIONNAIRE DETACHE' or x_ty == 'FONCTIONNAIRE MIS A DISPOSITION':
