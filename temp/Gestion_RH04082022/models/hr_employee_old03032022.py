...
class HrProrata(models.Model):
    ...
    x_total_indemnites_prorata = fields.Float(string='Total indemnités', required=False, compute="depend_field_prorata" ,store=True)
    x_remuneration_total_prorata = fields.Float(string='Remuneration totale', readonly=True,compute="renum_total_field_p",store=True)
    x_salaire_brut_p = fields.Float(string='Salaire brut', required=False, compute="calcul_salaire_brut_p" ,store=True)
    x_abattement_forfaitaire_p = fields.Float(string='Abattement Forfaitaire', required=False, compute="calcul_abattement_p" ,store=True)
    ...
    mnt_prorata_resp_exo = fields.Float(string='Exo.Resp.', required=False, compute='calcul_exoneration_responsabilite_p')
    ...
    mnt_prorata_astr_exo = fields.Float(string='Exo.Astreinte.', required=False, compute='calcul_exoneration_astreintes_p')
    ...
    mnt_prorata_techn_exo = fields.Float(string='Exo.Technicite.', required=False, compute='calcul_exoneration_technicites_p')
    ...
    mnt_prorata_inf_exo = fields.Float(string='Exo.Informatique.', required=False, compute='calcul_exoneration_informatique_p')
    ...
    mnt_prorata_explot_exo = fields.Float(string='Exo.Exploit-Reseaux.', required=False, compute='calcul_exoneration_informatique_p')
    ...
    mnt_prorata_resp_financ_exo = fields.Float(string='Exo.Resp.Financière.', required=False, compute='calcul_exoneration_resp_finance_p')
    ...
    mnt_prorata_sujetion_exo = fields.Float(string='Exo.Sujétion.', required=False, compute='calcul_exoneration_sujetion_p') 
    ...
    mnt_prorata_formation_exo = fields.Float(string='Exo.Formation.', required=False, compute='calcul_exoneration_formation_p')
    ...
    mnt_prorata_ifc_exo = fields.Float(string='Exo.IFC.', required=False, compute='calcul_exoneration_specifiques_ifc_p')
    ...
    x_net_payer_ctrct_p = fields.Float(store=True, string="Salaire NET", readonly=True, compute='net_payer_ctrct_field_p')
    ...
    def action_prorata_ok(self):
        for vals in self:
            if vals.tout_coch_prorata == True:
                vals.mnt_prorata_salaire = round((vals.nbre_jour_effectif * vals.x_solde_indiciaire_ctrct) / vals.nbre_jour_reel)
                ...
                vals.mnt_prorata_it = round((vals.nbre_jour_effectif * vals.x_indem_spec_inspect_trav) / vals.nbre_jour_reel)
                ...
                vals.mnt_prorata_resp_financ = round((vals.nbre_jour_effectif * vals.x_indem_finance) / vals.nbre_jour_reel)
                ...
                vals.mnt_prorata_ifc = round((vals.nbre_jour_effectif * vals.x_indem_spec_inspect_ifc) / vals.nbre_jour_reel)
                vals.mnt_prorata_irp = round((vals.nbre_jour_effectif * vals.x_indem_spec_inspect_irp) / vals.nbre_jour_reel)
                vals.mnt_prorata_ish = round((vals.nbre_jour_effectif * vals.x_indem_spec_inspect_ish) / vals.nbre_jour_reel)
            ...
            if vals.ish_coch_p == True:
                vals.mnt_prorata_irp = round((vals.nbre_jour_effectif * vals.x_indem_spec_inspect_ish) / vals.nbre_jour_reel)
    ...
    def renum_total_field_p(self):
        for val in self:
            ...
            val.x_remuneration_total_prorata = round(val.mnt_prorata_salaire + val.x_total_indemnites_prorata + val.allocation_prorata)
    ...
    @api.depends('mnt_prorata_veste_exo', 'mnt_prorata_caisse_exo', 'mnt_prorata_formation_exo', 'mnt_prorata_risque_exo',
    ...
    @api.depends('employee_id.x_type_employe_id', 'x_remuneration_total_prorata', 'x_mnt_cnss_p', 'x_mnt_carfo_p', 'x_iuts_net_p')
...
class HrEmployees(models.Model):
    ...
    def total_retenue_field(self):
        ...
        for val in self:
            val_id = self.id
            self.env.cr.execute("select count(p.id) from hr_precompte p, hr_employee h where h.id = p.x_employees_id and p.etat_prcpte='active' and p.x_precompte > 0  and p.x_employees_id = %d;" %(val_id))
            actif = self.env.cr.fetchone()
            var = actif
            if var >= 1 :
                self.env.cr.execute("select sum(p.x_retrait_prcpt_mois) from hr_precompte p, hr_employee h where h.id = p.x_employees_id and p.x_employees_id = %d" %(val_id))
                lo = self.env.cr.fetchone()
                x_retrait_prcpt_mois = lo
                #self.env.cr.execute("select p.x_precompte from hr_precompte p, hr_employee h where h.id = p.x_employees_id and p.x_employees_id = %d" %(val_id))
                #los = self.env.cr.fetchone()
                #self.x_precompte = los
                ...
                if val.x_type_employe_id.name == 'Fonctionnaire Detaché' or val.x_type_employe_id.name == 'fonctionnaire detaché' or val.x_type_employe_id.name == 'FONCTIONNAIRE DETACHE' or val.x_type_employe_id.name == 'Fonctionnaire Mis à Disposition' or val.x_type_employe_id.name == 'FONCTIONNAIRE MIS A DISPOSITION':
                    val.mnt_total_retenues = round(val.x_mnt_carfo + val.x_iuts_net + val.mnt_total_trop_percu + x_retrait_prcpt_mois)
                    ...
                    val.mnt_total_retenues = round(val.x_mnt_cnss + val.x_iuts_net + val.mnt_total_trop_percu + x_retrait_prcpt_mois)
                else:
                    val.mnt_total_retenues = round(val.x_mnt_taux_retenu_emolmt + val.mnt_total_trop_percu + x_retrait_prcpt_mois + val.x_iuts_net)
            else:
                if val.x_type_employe_id.name == 'Fonctionnaire Detaché' or val.x_type_employe_id.name == 'fonctionnaire detaché' or val.x_type_employe_id.name == 'FONCTIONNAIRE DETACHE' or val.x_type_employe_id.name == 'Fonctionnaire Mis à Disposition' or val.x_type_employe_id.name == 'FONCTIONNAIRE MIS A DISPOSITION':
    ...
    def total_prcpt_fields(self):
        for val in self:
            ...
            if val.company_id != False:
                val.env.cr.execute("select sum(p.x_retrait_prcpt_mois) from hr_precompte p, hr_employee h where h.id = p.x_employees_id and p.x_employees_id = %d" % (val_id))
    ...
    x_indem_spec_inspect_trav = fields.Float(store=True,string="Indemn.Spécifique Inspecteur de Travail", readonly=True)
    x_indem_spec_inspect_ifc = fields.Float(store=True, string="Indemn.Spécifique Forfaitaire Compensatrice", readonly=True)
    x_indem_spec_inspect_irp = fields.Float(store=True,string="Indemn.Spécifique de Responsabilité Pécunière", readonly=True)
    ...
    # Total indemnités = Total de toutes les indemnités auxquelles l'employé à droits
    x_total_indemnites = fields.Float(string="Total Indemn", readonly=True, compute='depend_field' , store=True)
    ...
    # Retenue IUTS
    x_retenue_iuts = fields.Float(string="Retenue IUTS", readonly=False, compute='retenue_iuts_field')

    # Montant charge
    x_montant_charge = fields.Float(string="Montant charge", readonly=True, compute='mnt_charge_field')

    # IUTS Net
    x_iuts_net = fields.Float(string="IUTS Net", readonly=True, compute='net_iuts_fields')

    # Net à payer fonctionnaire
    x_net_payer = fields.Float(string="Net à payer", readonly=False)
    ...
    def calcul_exoneration_responsabilite(self):
        ...
        for vals in self:
            ...
            vals.x_indem_resp_exo = min([premiere_limite_res, vals.x_indem_resp, 50000])
    ...
    @api.onchange('x_type_employe_id', 'x_solde_indiciaire_ctrct', 'x_total_indemnites', 'x_emolument_ctrct_net',
                  'x_allocation_familial', 'mnt_total_rappel')
    def renum_total_field(self):
        for val in self:
            # val.x_remuneration_total = round(val.x_solde_indiciaire_ctrct + val.x_total_indemnites + val.x_emolument_ctrct_net + val.x_allocation_familial)
            val.x_remuneration_total = round(
                val.x_solde_indiciaire_ctrct + val.x_total_indemnites + val.x_emolument_ctrct_net + val.mnt_total_rappel + val.x_allocation_familial)
    ...
    # fonction qui permet d'avoir le montant cnss à partir du salaire de base
    @api.onchange('x_type_employe_id', 'x_remuneration_total')
    ...
    @api.onchange('x_type_employe_id', 'x_remuneration_total', 'x_mnt_carfo', 'x_iuts_net', 'mnt_total_rappel',
                  'mnt_total_trop_percu')
    def net_payer_field(self):
        for val in self:
            val.x_net_payer = round(
                val.x_remuneration_total - val.x_iuts_net - val.x_mnt_carfo - val.mnt_total_trop_percu)
...
class HrEmployee(models.Model):
    ...
    #self.reinitialiser_total_rappel()
    # Fonction permettant de concatener pour obtenir la classification globale de l'employé dans la fonction publique
    @api.onchange('x_categorie_id', 'x_echelle_id', 'x_echellon_id')
    def _result_conc(self):
        for concat in self:
    ...
    # calcul abattement forfaitaire
    @api.depends('x_solde_indiciaire_ctrct', 'mnt_rappel_salaire')
    def calcul_abattement(self):
        for vals in self:
            type_emp = vals.x_lib_type_emp
            x_cat = vals.x_categorie_c_id.name
            x_cat_f = vals.x_categorie_id.name
            x_solde_indiciaire_ctrct = vals.x_solde_indiciaire_ctrct
            if vals.mnt_rappel_salaire:
                x_solde_indiciaire_ctrct = vals.x_solde_indiciaire_ctrct + vals.mnt_rappel_salaire
            if type_emp == "Contractuel" or type_emp == "CONTRACTUEL":
                if x_cat == '1' or x_cat == '2' or x_cat == '6':
                    vals.x_abattement_forfaitaire = round((x_solde_indiciaire_ctrct * 20) / 100)
                else:
                    vals.x_abattement_forfaitaire = round((x_solde_indiciaire_ctrct * 25) / 100)
            else:
                if x_cat_f == 'A' or x_cat_f == 'B' or x_cat_f == 'P':
                    vals.x_abattement_forfaitaire = round((x_solde_indiciaire_ctrct * 20) / 100)
                else:
    ...
    def renum_total_field(self):
        for val in self:
            # val.x_remuneration_total = round(val.x_solde_indiciaire_ctrct + val.x_total_indemnites + val.x_emolument_ctrct_net + val.x_allocation_familial)
            val.x_remuneration_total = round(
                val.x_solde_indiciaire_ctrct + val.x_total_indemnites + val.x_emolument_ctrct_net +  val.x_allocation_familial + val.mnt_total_rappel)
    ...
    # fonction qui permet d'avoir le montant net à payer pour les contractuels 1
    @api.depends('x_type_employe_id', 'x_remuneration_total', 'x_mnt_cnss', 'x_mnt_carfo', 'x_iuts_net',
                 'mnt_total_rappel', 'mnt_total_trop_percu', 'x_retrait_prcpt_mois','mnt_rappel_salaire_net')
    def net_payer_ctrct_field(self):
        for val in self:
