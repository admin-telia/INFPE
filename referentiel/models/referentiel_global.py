...
class HrUsers(models.Model):
    ...
    _inherit = "res.users"
    x_exercice_id = fields.Many2one('ref_exercice',string = 'Choisir Exercice', required=True, default=lambda self: self.env['ref_exercice'].search([('etat','=', 1)]))
...
class temp_change_exo_user(models.Model):
    ...
    x_exercices_id = fields.Many2one('ref_exercice',string='Exercice',default=lambda self: self.env['ref_exercice'].search([('etat','=', 1)]), required=True)
