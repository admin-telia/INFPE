        ...
        if not (no_ecr):
            ...
            for val in self.env.cr.dictfetchall():
                ...
                if etat == True:
                    ...
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s) """,(no_ecr, no_lecr, v_cred, v_mnt, val_ex, v_str, val_date, v_sens, v_etat))
                    ...
                    VALUES(%s, %s, %s, %s, %s, %s, %s, %s, 'P')""",(no_ecr, no_lecrs, var_cptesdebs, v_mnt, var_sens, val_ex, v_str, val_date))
    ...
    def gen_ecr_libr(self):
        for val in self:
            ...
            if not(no_ecr):
                ...
                val.env.cr.execute("""INSERT INTO compta_compteur_ecr(x_exercice_id,company_id,no_ecr,no_lecr) VALUES(%d, %d, %d, %d)""" %(val_struct,val_ex,val.no_ecr, record.no_lecr))
            else:
                val.no_ecr = no_ecr + 1
                for record in val.ecriture_libre_ids:
                    no_ecrs1 = no_ecrs1 + 1
                    record.no_lecr = no_ecrs1
                val.env.cr.execute("UPDATE compta_compteur_ecr SET no_ecr = %d, no_lecr = %d WHERE x_exercice_id = %d and company_id = %d" %(val.no_ecr,record.no_lecr,val_ex,val_struct))
...
class Compta_Paiement(models.Model):
    ...
    """
    _sql_constraints = [
        ('reference_uniq', 'unique (reference)', "Ce numéro de chèque est déjà passé.")
    ]"""
            ...
            for val in self.env.cr.dictfetchall():
                ...
                if var_cptes == 0:
                    raise ValidationError(_(
                        "Choisissez encore le même REF_MDT ou REF_OP en cliquant dessus"))
                else:
                    ...
                    no_ecr, no_lecr, var_cptes, type_pj, ref_pj, mnt, v_ex, val_struct, dte, fg_sens))
...
class Compta_Detail_Ordre_Virement(models.Model):
    ...
    def MontantMandat(self):
        for val in self:
            ...
            if val.code == '31':
                ...
                val.env.cr.execute(
                    "select mnt_ord from budg_mandat where id = %d and company_id = %d and x_exercice_id = %d and state = 'E' " % (
                ...
                val.env.cr.execute(
                    "select no_mandat from budg_mandat where id = %d and company_id = %d and x_exercice_id = %d and state = 'E' " % (
                        val_mdt, val_struct, val_ex))
                ...
                val.env.cr.execute(
                    "select imput_benef from budg_mandat where id = %d and company_id = %d and x_exercice_id = %d and state = 'E' " % (
                        val_mdt, val_struct, val_ex))
    ...
    def MontantOp(self):
        ...
        for val in self:
            ...
            if val.code == '30':
                ...
                val.env.cr.execute(
                    "select mnt_op from budg_op where id = %d and company_id = %d and x_exercice_id = %d and et_doss = 'C' " % (
                ...
                val.env.cr.execute(
                    "select no_op from budg_op where id = %d and company_id = %d and x_exercice_id = %d and et_doss = 'C' " % (
                        val_op, val_struct, val_ex))
                ...
                val.env.cr.execute(
                    "select imput_benefs from budg_op where id = %d and company_id = %d and x_exercice_id = %d and et_doss = 'C' " % (
                        val_op, val_struct, val_ex))
                cpte = val.env.cr.fetchone()
                cpte1 = cpte and cpte[0] or 0
    ...
    def get_imputationbq(self):
        for val in self:
            ...
            # Nouvelle requete
            val.env.cr.execute(
                "select r.souscompte_id, r.fg_term from compta_type_op_cpta r where r.typebase_id = %s and type_opcpta1 = %s",
                (val_type1, val_type2))
...
class Journal_ModReg(models.Model):
    ...
    no_imputation = fields.Many2one("compta_plan_lines", 'Compte de règlement', domain="[('company_id','=',company_id)]", required = True)
...
class Compta_op_releve(models.Model):
    ...
    def generer_ecriture_old(self):
        ...
        self.env.cr.execute("select * from compta_op_releve where x_exercice_id = %d and company_id = %d and id = %d" %(val_ex,val_struct, id_releve))
        ...
        self.env.cr.execute("select * from compta_op_releve_lines where x_exercice_id = %d and company_id = %d and op_releve_id = %d " %(val_ex,val_struct, id_releve))
        curs = self.env.cr.dictfetchall()
        ...
        FROM compta_op_releve_lines WHERE x_exercice_id = %d AND company_id = %d AND op_releve_id = %d """ %(val_ex, val_struct, id_releve))
...
class Compta_balance_reel(models.Model):
    ...
    def get_report_values(self):
        ...
        val_fin = self.periode_fin

        docclasse = []
        self.env.cr.execute("SELECT classe, libelle, debiteur, crediteur FROM vue_classe WHERE company_id = %s and x_exercice_id = %s and dte BETWEEN '%s' and '%s' " %(company_id, x_exercice_id, val_deb, val_fin))
        for line in self.env.cr.dictfetchall():
            classe = line['classe']
            libelle = line['libelle']
            v_debclasse = int(line['debiteur'])
            v_credclasse = int(line['crediteur'])

            # Boucle 2
            docsousclasse = []
            self.env.cr.execute("SELECT classe, sousclasse, libelle, debiteur, crediteur FROM vue_sousclasse WHERE classe = '%s' and company_id = %s and x_exercice_id = %s and dte BETWEEN '%s' and '%s' " %(classe, company_id, x_exercice_id, val_deb, val_fin))
            for line in self.env.cr.dictfetchall():

                sousclasse = line['sousclasse']
                libelle = line['libelle']
                v_debsousclasse = int(line['debiteur'])
                v_credsousclasse = int(line['crediteur'])

                # Boucle 3
                doccompte = []
                self.env.cr.execute("SELECT classe, sousclasse ,cpte, libelle, debiteur , crediteur FROM vue_compte WHERE classe = '%s' and sousclasse = '%s' and company_id = %s and x_exercice_id = %s and dte BETWEEN '%s' and '%s' " %(classe, sousclasse, company_id, x_exercice_id, val_deb, val_fin))
                for line in self.env.cr.dictfetchall():

                    compte = line['cpte']
                    v_debcompte = line['debiteur']
                    v_credcompte = line['crediteur']
                    libelle = line['libelle']

                    # Boucle 4
                    docsouscompte = []
                    self.env.cr.execute("SELECT classe, sousclasse ,cpte, souscpte, libelle, debiteur ,crediteur FROM vue_souscompte WHERE classe = '%s' and sousclasse = '%s' and cpte = '%s' and company_id = %s and x_exercice_id = %s and dte BETWEEN '%s' and '%s' " %(classe, sousclasse, compte, company_id, x_exercice_id, val_deb, val_fin))
                    for line in self.env.cr.dictfetchall():

                        souscompte = line['souscpte']
                        v_debsouscompte = line['debiteur']
                        v_credsouscompte = line['crediteur']
                        libelle = line['libelle']

                        esouscomptes = {souscompte: [v_debsouscompte, v_credsouscompte, libelle]}
                        esouscompte = OrderedDict(sorted(esouscomptes.items(), key=lambda t: t[0]))
                        docsouscompte.append(esouscompte)

                    ecomptes = {compte: [v_debcompte, v_credcompte, libelle],"ssouscompte": docsouscompte}
                    ecompte = OrderedDict(sorted(ecomptes.items(), key=lambda t: t[0]))
                    doccompte.append(ecompte)

                esousclasses = {sousclasse: [v_debsousclasse, v_debsousclasse, libelle],"ccompte": doccompte}
                esousclasse = OrderedDict(sorted(esousclasses.items(), key=lambda t: t[0]))
                docsousclasse.append(esousclasse)

            eclasses = {classe: [v_debclasse, v_debclasse, libelle], "sclasse": docsousclasse}
            eclasse = OrderedDict(sorted(eclasses.items(), key=lambda t: t[0]))
            docclasse.append(eclasse)
    ...
    def chercher_ecriture(self):
        ...
        if self.no_lecr:
            for vals in self:
                ...
                from compta_ligne_ecriture l,  ref_souscompte r where r.id = l.no_souscptes and
                l.fg_etat in ('W','V','P','R') and l.no_lecr = %s and l.company_id = %s and l.x_exercice_id = %s
                group by r.souscpte, r.lb_long, sens, ligne, mnt, etat, type_pj order by libelle asc""",
                                    (val_lecr, val_struct, val_ex))
                rows = vals.env.cr.dictfetchall()
                result = []

                vals.edition_ecriture_line.unlink()
                for line in rows:
                    result.append((0, 0, {'noligne': line['ligne'], 'cpte': line['libelle'], 'fg_sens': line['sens'],
        ...
        if self.ecr:
            for vals in self:
                ...
                and l.company_id = %s and l.x_exercice_id = %s group by r.souscpte, r.lb_long, sens, ligne, mnt, etat, type_pj order by libelle asc""",
            ...
            for vals in self:
                vals.env.cr.execute("""select l.no_lecr as ligne, concat(r.souscpte, ' ', r.lb_long) as libelle,l.fg_sens as sens, 
                l.mt_lecr as mnt, l.fg_etat as etat, l.type_pj as piece
                from compta_ligne_ecriture l, ref_souscompte r
                where r.id = l.no_souscptes and l.fg_etat in ('W','V','P','R')
                and l.company_id = %s and l.x_exercice_id = %s
                group by r.souscpte, r.lb_long, ligne, sens, mnt, etat, type_pj order by l.no_lecr""",
                                    (val_struct, val_ex))
                rows = vals.env.cr.dictfetchall()
                result = []
...
class Compta_balance_generale(models.Model):
    ...
    def afficher_balance(self):
        ...
        for vals in self:
            vals.env.cr.execute(
                """ select l.souscpte as scpte, initcap(r.lb_long) as lib, r.souscpte as cpte from compta_plan_lines l, ref_souscompte r where r.id = l.souscpte order by r.souscpte """)
            ...
            for line in rows:
                result.append((0, 0, {'cpte': line['cpte'], 'souscpte': line['scpte'], 'libelle': line['lib']}))
            self.balance_generale_line = result

        self.Calcul()

    def UniqueLigneServie(self):
        ...
        for vals in self:
            vals.env.cr.execute(
                """ select distinct l.souscpte as scpte, initcap(r.lb_long) as lib, r.souscpte as cpte from compta_plan_lines l, ref_souscompte r, 
                compta_ligne_ecriture e where r.id = l.souscpte and r.id = e.no_souscptes order by r.souscpte; """)
    ...
    def Calcul(self):
        ...
        for vals in self.env.cr.dictfetchall():
            ...
            and l.company_id = %d and l.x_exercice_id = %d and l.fg_etat not in ('A','R') and l.mt_lecr <> 0 group by r.souscpte, r.lb_long order by compte asc""" % (
            cpte, val_struct, val_ex))
...
class Compta_parametrage_line(models.Model):
    ...
    cpte = fields.Many2one('compta_plan_lines', 'Compte', required=True, domain="[('company_id','=', company_id)]")
    cptes = fields.Integer()
...
class ComptaReversement(models.Model):
    ...
    def initialiser(self):
        ...
        self.env.cr.execute(
            """select no_jour as jour from compta_jour_guichet where dt_ouvert = %s and company_id = %s and cd_us_gui = %s""",
            (v_dte, val_struct, v_id))
        ...
        for vals in self:
            ...
            and r.id = u.gui_us and l.company_id = %s and u.date_ope = %s and l.fg_etat <> 'A' group by r.id,p.id, u.no_jour """,
                                (v_jr, v1_id, val_struct, v_dte))
            rows = vals.env.cr.dictfetchall()
            result = []
            ...
            for line in rows:
                result.append((0, 0,
                               {'code': line['code'], 'guichetier': line['guichetier'], 'encours': line['encours'],
                                'no_jour': line['jour']}))
    ...
    def reverser(self):
        ...
        self.env.cr.execute(
            "select max(no_oper + 1) from compta_versement_guichet where x_exercice_id = %d and company_id = %d and no_jour_guichet = %d and no_jour_caisse =%d and guichetier = %d" % (
            val_ex, val_struct, v_jr, v_jr, v1_id))
        res = self.env.cr.fetchone()
        ...
        for record in self.reversement_ids:
            ...
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s)""",
                                (val_ex, val_struct, v_jr, v_jr, val_op, v_id, record.mntverser, record.mnt_rest))
...
class ComptaGuichetUnique(models.Model):
    ...
    def action_confirmer(self):
        ...
        self.env.cr.execute("select distinct g.state from compta_jour_guichet g, compta_guichet_unique u where u.date_ope = g.dt_ouvert and u.company_id = %s and g.state = 'O'",(val_struct))
        et = self.env.cr.fetchone()
        etat = et and et[0] or 0
        ...
        if etat != 'O':
            ...
            for vals in self.guichet_line:
                ...
                if vals.pj.libelle.refe == '62':
                    self.env.cr.execute(
                        "update budg_titrerecette set et_doss = 'F' where x_exercice_id = %s and company_id = %s and cd_titre_recette = %s",
                        (val_ex, val_struct, val_t))
                if vals.pj.libelle.refe == '31':
                    self.env.cr.execute(
                        "update budg_mandat set state = 'F' where x_exercice_id = %s and company_id = %s and no_mandat = %s",
                        (val_ex, val_struct, val_t))
...
class ComptaFermeCaisse(models.Model):
    ...
    def mntOuvert(self):
        ...
        self.env.cr.execute(
            "select distinct g.no_jour from compta_jour_guichet g, compta_guichet_unique u where u.date_ope = g.dt_ouvert and g.dt_ouvert = %s and u.company_id = %s",
            (v_dte, val_struct))
        jr = self.env.cr.fetchone()
        self.no_jour = jr and jr[0] or 0
    ...
    def action_afficher(self):
        ...
        self.env.cr.execute(
            "select count(cd_us_caisse)from compta_caisse_struct where fg_resp = True and cd_us_caisse = %s and company_id = %s",
            (v_id, val_struct))
        resul = self.env.cr.fetchone()
        ...
        for vals in self:
            vals.env.cr.execute("""select distinct j.cd_us_gui as guichetier, j.mnt_ouverture as ouverture, j.state as etat, 
                ...
                where l.x_exercice_id = %s and l.company_id = %s and gu.date_ope = j.dt_ouvert and j.cd_us_gui = gu.gui_us and gu.id = l.guichet_id and j.no_jour = gu.no_jour and gu.no_jour = %s and l.fg_etat <> 'A' group by j.cd_us_gui, j.mnt_ouverture, j.state"""
                                , (val_struct, v_ex, v_jr, val_struct, v_ex, v_jr, val_struct, v_ex, v_jr, val_struct,
                                   v_ex, v_jr, v_ex, val_struct, v_jr))
        ...
        self.env.cr.execute("""select coalesce(sum(case when gu.type_operation = 1 then l.mnt_op_cpta end),0) - coalesce(sum(case when gu.type_operation = 2 then l.mnt_op_cpta end),0) as solde 
            ...
            gu.id = l.guichet_id and gu.no_jour = %s and gu.modreg = '0' and l.fg_etat <> 'A' """,
                            (val_ex, val_struct, v_jr))
        ...
        self.env.cr.execute("""select sum(l.mnt_op_cpta) from compta_guichet_line l, compta_guichet_unique g 
            where g.id = l.guichet_id and g.company_id = %d and g.x_exercice_id = %d and g.no_jour = %d and l.fg_etat <> 'A' ;""" % (
    ...
    def fermer_guichet(self):
        ...
        for x in self.ferme_caisse_line:
            ...
            self.env.cr.execute("""UPDATE compta_jour_guichet SET state ='F', mnt_fermeture = %s, dt_fermeture = %s WHERE
                state ='O' and cd_us_gui = %s and company_id = %s and x_exercice_id = %s""",
                                (v_mnt, v_dte, val_user, val_struct, val_ex))
        ...
        for line in self.env.cr.dictfetchall():
            ...
            if etat == 'C':
                ...
                if not (no_ecr):
                    ...
                    lecr = no_ecrs1
                    self.env.cr.execute(
                        "INSERT INTO compta_ecriture(no_ecr, dt_ecriture,type_ecriture, type_op ,x_exercice_id, company_id, state) VALUES (%s, %s, %s, 'G', %s, %s, 'P')",
                        (no_ecr, v_dte, type_ecr, val_ex, val_struct))

                    self.env.cr.execute("""INSERT INTO compta_ligne_ecriture(no_ecr,no_lecr, no_souscptes, mt_lecr, x_exercice_id, company_id, fg_sens, dt_ligne, fg_etat) 
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, 'P') """,
                                        (no_ecr, lecr, cpte, v_mnt, val_ex, val_struct, sens, v_dte))

                    self.env.cr.execute(
                        """INSERT INTO compta_compteur_ecr(x_exercice_id,company_id,no_ecr,no_lecr) VALUES(%d, %d, %d, %d)""" % (
                        val_ex, val_struct, no_ecr, lecr))

                    self.env.cr.execute(
                        "UPDATE compta_guichet_unique SET state = 'P' WHERE x_exercice_id = %d and company_id = %d and id = %d" % (
                        val_ex, val_struct, v_id))

                    self.env.cr.execute(
                        "UPDATE compta_guichet_line SET no_ecr = %s where company_id = %s and x_exercice_id = %s and dte_op = %s",
                        (no_ecr, val_struct, val_ex, v_dte))
                    ...
                    self.env.cr.execute("""select l.* from compta_guichet_line l, compta_guichet_unique u where u.company_id = %s and u.x_exercice_id = %s 
                        and l.guichet_id = u.id and l.guichet_id = %s and mnt_op_cpta > 0 and u.state = 'P' and l.fg_etat = 'P' """,
                                        (val_struct, val_ex, v_id))
                    for res1 in self.env.cr.dictfetchall():
                        ...
                        if etat != 'P':
                            ...
                            if tva == True:
                                self.env.cr.execute("""INSERT INTO compta_ligne_ecriture(no_ecr, no_lecr, no_souscptes, mt_lecr, fg_sens, x_exercice_id, company_id, dt_ligne,fg_etat) 
                                    VALUES(%s, %s, %s, %s, %s, %s, %s, %s, 'P')""", (
                                no_ecr, no_lecr, id_imput_tva, mnt_tva, lsens, val_ex, val_struct, v_dte))

                                no_lecrs = no_lecr + 1
                                self.env.cr.execute("""INSERT INTO compta_ligne_ecriture(no_ecr, no_lecr, no_souscptes, mt_lecr, fg_sens, x_exercice_id, company_id, dt_ligne,fg_etat) 
                                    VALUES(%s, %s, %s, %s, %s, %s, %s, %s, 'P')""",
                                                    (no_ecr, no_lecrs, imput, mnt, lsens, val_ex, val_struct, v_dte))

                                self.env.cr.execute(
                                    "UPDATE compta_compteur_ecr SET no_ecr = %d, no_lecr = %d WHERE x_exercice_id = %d and company_id = %d" % (
                                    no_ecr, no_lecrs, val_ex, val_struct))

                            else:
                                self.env.cr.execute("""INSERT INTO compta_ligne_ecriture(no_ecr, no_lecr, no_souscptes, mt_lecr, fg_sens, x_exercice_id, company_id, dt_ligne,fg_etat) 
                                    VALUES(%s, %s, %s, %s, %s, %s, %s, %s, 'P')""",
                                                    (no_ecr, no_lecr, imput, mnt, lsens, val_ex, val_struct, v_dte))
                            ...
                            self.env.cr.execute(
                                "UPDATE compta_guichet_line SET fg_etat = 'V' where guichet_id = %s and company_id = %s and x_exercice_id = %s and id = %s",
                    ...
                    self.env.cr.execute("""select distinct l.* from compta_guichet_line l, compta_guichet_unique u where l.company_id = %s and l.x_exercice_id = %s 
                        and u.id = l.guichet_id and l.guichet_id = %s and l.mnt_op_cpta > 0 and l.fg_etat = 'P' and u.state = 'P' """,
                                        (val_struct, val_ex, v_id))
                    for res1 in self.env.cr.dictfetchall():
                        print("resulat ligne", res1)
                        mnt = res1['mnt_ht']
    ...
    def fermer_caisse(self):
        ...
        self.env.cr.execute(
            "select * from compta_jour_guichet where dt_ouvert = %s and state = 'O' and company_id = %s",
            (val_dte, val_struct))
        resultat = self.env.cr.fetchone()
        ...
        if res > 0:
            raise ValidationError(
                _('Oups!!! Tous les guichets ne sont pas fermés. Cliquez sur le boutton Fermer guichets non fermés'))
        ...
        self.env.cr.execute(
            """UPDATE compta_jour_caisse SET dt_fermeture = %s, mnt_fermeture = %s WHERE dt_ouvert = %s and company_id = %s""",
            (val_dte, somme, val_dte, val_struct))
...
class ComptaFermeGuichet(models.Model):
    ...
    def action_afficher(self):
        ...
        for vals in self:
            ...
            for line in rows:
                result.append((0, 0, {'categop': line['cat'], 'typeop': line['typ2'], 'typepiece': line['piece'],
                                      'refp': line['refe'], 'modreg': line['reg'], 'intervenant': line['nom'],
                                      'annee': line['exo'], 'mt_encaisse': line['encaisse'],
        ...
        from compta_guichet_line l, compta_guichet_unique gu
        where gu.id = l.guichet_id and l.x_exercice_id = %s and l.company_id = %s and gu.gui_us = %s and gu.no_jour = %s and l.fg_etat <> 'A' """,
                            (val_ex, val_struct, v_id, v_jr))
        ...
        and gu.gui_us = %s and gu.no_jour = %s and modreg='0' and l.fg_etat <> 'A' """,
                            (val_ex, val_struct, v_id, v_jr))
    ...
    def fermer_guichet(self):
        ...
        self.env.cr.execute(
            """UPDATE compta_jour_guichet SET state = 'F', dt_fermeture = %s, mnt_fermeture = %s WHERE x_exercice_id = %s and company_id = %s and no_jour = %s and cd_us_gui = %s""",
            (v_date, v_mntfermeture, val_ex, val_struct, v_jr, v_id))
...
class ComptaBilletage(models.Model):
    ...
    def action_valider(self):
        ...
        self.env.cr.execute(
            "select mnt from compta_billetage where no_jour = %d and guichetier = %d and company_id = %d and x_exercice_id = %d" % (
            v_jr, v1_id, val_struct, val_ex))
        mont = self.env.cr.fetchone()
        montant = mont and mont[0] or 0

        self.env.cr.execute(
            "select sum(mnt) from compta_billetage_line where billetage_id = %d and company_id = %d and x_exercice_id = %d" % (
            val_id, val_struct, val_ex))
        tot = self.env.cr.fetchone()
        total = tot and tot[0] or 0

        if total != montant:
            raise ValidationError(
                _("Le montant total du billetage est différent du montant à la fermeture de votre guichet."))
    ...
    def initialiser(self):
        ...
        self.env.cr.execute(
            "select no_jour, cd_us_caisse, dt_ouvert from compta_jour_caisse where dt_ouvert = '%s' and x_exercice_id = %s " % (
            vr_dte, val_ex))
        ...
        if dte1 != vr_dte:
            ...
            self.env.cr.execute("""select sum( case when u.type_operation = 1 then l.mnt_op_cpta end) - coalesce(sum( case when u.type_operation = 2 then l.mnt_op_cpta end),0) as encours
                ...
                and l.company_id = %s and l.x_exercice_id = %s and u.gui_us = %s and modreg = '0' and l.fg_etat <> 'A' """,
                                (v_jr, val_struct, val_ex, v_id))
            mt = self.env.cr.fetchone()
...
class ComptaTeneurCompteLine(models.Model):
    ...
    compte = fields.Many2one("compta_plan_lines", "Compte",domain="[('company_id','=',company_id)]", required=True)
    libelle = fields.Char("Libelle", readonly=True)
    sens = fields.Selection([
        ('D', 'Débit'),
        ('C', 'Crédit'),
...
class ComptaEncours(models.Model):
    ...
    def afficher(self):
        ...
        for vals in self:
            ...
            where u.id = l.guichet_id and u.no_jour = %s and l.company_id = %s and u.date_ope = %s and l.fg_etat <> 'A' and u.state not in ('draft','A')""",
                                (v_jr, val_struct, v_dte, v_jr, val_struct, v_dte, v_jr, val_struct, v_dte, v_jr,
                                 val_struct, v_dte, v_jr, val_struct, v_dte, v_jr, val_struct, v_dte, v_jr, val_struct,
                                 v_dte, v_jr, val_struct, v_dte))
            rows = vals.env.cr.dictfetchall()
            result = []
...
class ComptaCorrectionOpGuichet(models.Model):
    ...
    def corriger(self):
        ...
        for vals in self.correction_line:
            ...
            if vals.ok == True and vals.tva == True:
            ...
            if vals.ok == True and vals.tva == False:
...
class VueClasse(models.Model):
    ...
    classe =fields.Many2one('ref_cl_cpt_pcg',string="Classe", readonly=True)
    debiteur=fields.Float('',readonly=True)
    crediteur = fields.Float('', readonly=True)
    dte = fields.Date("Date", readonly=True)
    ...
    def init(self):
        ...
        tools.drop_view_if_exists(self.env.cr, 'vue_classe')

        self.env.cr.execute("CREATE OR REPLACE VIEW vue_classe AS (select l.company_id,l.x_exercice_id,rc.cl_cpt_pcg as classe, UPPER(rc.lb_long) as libelle, coalesce(sum( case when l.fg_sens = 'D' then l.mt_lecr end),0) as debiteur,coalesce(sum( case when l.fg_sens = 'C' then l.mt_lecr end),0) as crediteur, l.dt_ligne as dte FROM compta_ligne_ecriture l,ref_classe_pcg rc, ref_sousclasse_pcg rsc, ref_souscompte rs,compta_plans cp, compta_plan_lines cpl WHERE rc.id = rsc.cl_cpt_pcg_id and rsc.id = cp.sousclasse_id and cp.id = cpl.plan_id and rs.id = cpl.souscpte and cpl.souscpte = l.no_souscptes GROUP by l.company_id,l.x_exercice_id, rc.cl_cpt_pcg, rc.lb_long, l.dt_ligne order by rc.cl_cpt_pcg, l.dt_ligne)")


class VueSousClasse(models.Model):
    _name="vue_sousclasse"
    _auto=False
    company_id = fields.Many2one('res.company', default=lambda self: self.env.user.company_id.id,readonly=True)
    x_exercice_id = fields.Many2one("ref_exercice",
									default=lambda self: self.env['ref_exercice'].search([('etat', '=', 1)]),
									string="Exercice", readonly=True)
    classe =fields.Many2one('ref_cl_cpt_pcg',string="Classe", readonly=True)
    sousclasse = fields.Many2one('ref_scl_cpt_pcg', string="Sous Classe", readonly=True)
    somsousclasse=fields.Float('',readonly=True)
    dte = fields.Date("Date", readonly=True)

    @api.model
    def init(self):

        tools.drop_view_if_exists(self.env.cr, 'vue_sousclasse')
        self.env.cr.execute("CREATE OR REPLACE VIEW vue_sousclasse AS (select l.company_id,l.x_exercice_id,rc.cl_cpt_pcg as classe, rsc.scl_cpt_pcg as sousclasse, UPPER(rsc.lb_long) as libelle, coalesce(sum( case when l.fg_sens = 'D' then l.mt_lecr end),0) as debiteur, coalesce(sum( case when l.fg_sens = 'C' then l.mt_lecr end),0) as crediteur, l.dt_ligne as dte FROM compta_ligne_ecriture l,ref_classe_pcg rc, ref_sousclasse_pcg rsc, ref_souscompte rs, compta_plans cp, compta_plan_lines cpl WHERE rc.id = rsc.cl_cpt_pcg_id and rsc.id = cp.sousclasse_id and cp.id = cpl.plan_id and rs.id = cpl.souscpte and cpl.souscpte = l.no_souscptes  GROUP by l.company_id,l.x_exercice_id,rc.cl_cpt_pcg, rsc.scl_cpt_pcg, rsc.lb_long, l.dt_ligne order by rc.cl_cpt_pcg, rsc.scl_cpt_pcg, l.dt_ligne)")


class VueCompte(models.Model):
    _name="vue_compte"
    _auto=False
    ...
    classe =fields.Many2one('ref_cl_cpt_pcg',string="Classe", readonly=True)
    sousclasse = fields.Many2one('ref_scl_cpt_pcg', string="Sous Classe", readonly=True)
    compte = fields.Many2one("ref_compte","Compte", readonly=True)
    somcompte = fields.Float('',readonly=True)
    dte = fields.Date("Date", readonly=True)

    @api.model
    def init(self):

        tools.drop_view_if_exists(self.env.cr, 'vue_compte')

        self.env.cr.execute("CREATE OR REPLACE VIEW vue_compte AS (select l.company_id,l.x_exercice_id,rc.cl_cpt_pcg as classe, rsc.scl_cpt_pcg as sousclasse,c.compte as cpte,UPPER(c.lb_long) as libelle, coalesce(sum( case when l.fg_sens = 'D' then l.mt_lecr end),0) as debiteur,coalesce(sum( case when l.fg_sens = 'C' then l.mt_lecr end),0) as crediteur, l.dt_ligne as dte FROM compta_ligne_ecriture l,ref_classe_pcg rc, ref_sousclasse_pcg rsc, ref_compte c, ref_souscompte rs, compta_plans cp, compta_plan_lines cpl WHERE rc.id = rsc.cl_cpt_pcg_id and rsc.id = cp.sousclasse_id and cp.id = cpl.plan_id and rs.id = cpl.souscpte and cpl.souscpte = l.no_souscptes and c.id = cpl.cpte GROUP by l.company_id,l.x_exercice_id,rc.cl_cpt_pcg, rsc.scl_cpt_pcg, c.compte,c.lb_long, l.dt_ligne order by c.compte, rc.cl_cpt_pcg, rsc.scl_cpt_pcg, l.dt_ligne)")


class VueSousCompte(models.Model):
    _name="vue_souscompte"
    _auto=False
    ...
    sousclasse = fields.Many2one('ref_scl_cpt_pcg', string="Sous Classe", readonly=True)
    compte = fields.Many2one("ref_compte","Compte", readonly=True)
    souscompte = fields.Many2one("ref_souscompte","Sous Compte", readonly=True)
    somsouscompte = fields.Float('',readonly=True)
    dte = fields.Date("Date", readonly=True)

    @api.model
    def init(self):

        tools.drop_view_if_exists(self.env.cr, 'vue_souscompte')
        self.env.cr.execute("CREATE OR REPLACE VIEW vue_souscompte AS (select l.company_id,l.x_exercice_id,rc.cl_cpt_pcg as classe, rsc.scl_cpt_pcg as sousclasse,c.compte as cpte,s.souscpte as souscpte, UPPER(s.lb_long) as libelle, coalesce(sum( case when l.fg_sens = 'D' then l.mt_lecr end),0) as debiteur,coalesce(sum( case when l.fg_sens = 'C' then l.mt_lecr end),0) as crediteur,l.dt_ligne as dte FROM compta_ligne_ecriture l,ref_classe_pcg rc, ref_sousclasse_pcg rsc, ref_compte c, ref_souscompte s, compta_plans cp, compta_plan_lines cpl WHERE rc.id = rsc.cl_cpt_pcg_id and rsc.id = cp.sousclasse_id and cp.id = cpl.plan_id and s.id = cpl.souscpte and cpl.souscpte = l.no_souscptes and c.id = cpl.cpte and s.id = cpl.souscpte GROUP by l.company_id,l.x_exercice_id,rc.cl_cpt_pcg, rsc.scl_cpt_pcg, c.compte,s.souscpte, s.lb_long, l.dt_ligne order by rc.cl_cpt_pcg, rsc.scl_cpt_pcg,c.compte,s.souscpte, l.dt_ligne)")
