...
class HrParametrageIndemniteSpecifiqueIT(models.Model):
    ...
    x_type_indem_id = fields.Many2one('hr_typeindemnite', string='Type Indemnité', required=True,
                                      default=lambda self: self.env['hr_typeindemnite'].search(
                                          [('name', '=', "Indemnité spécifique Admin Trésor")]), readonly=True)
    x_emploi_id = fields.Many2one('hr_emploi', string='Emploi', required=True)
    x_zone_id = fields.Many2one('hr_zone', string='Zone', required=False)
    x_taux = fields.Float(string='Taux à servir', required=True)
    x_echelle_c_id = fields.Many2one('hr_echelle', string='Echelle', required=False)
...
class HrParametrageIndemniteSpecifiqueIRP(models.Model):
    ...
    x_type_indem_id = fields.Many2one('hr_typeindemnite', string='Type Indemnité', required=True,
                                      default=lambda self: self.env['hr_typeindemnite'].search(
                                          [('name', '=', "Indemnité spécifique Harmonisée")]), readonly=True)
    x_emploi_id = fields.Many2one('hr_emploi', string='Emploi', required=True)
    x_zone_id = fields.Many2one('hr_zone', string='Zone', required=True)
    x_taux = fields.Float(string='Taux à servir', required=True)
    x_echelle_c_id = fields.Many2one('hr_echelle', string='Echelle', required=True)
...
class RefService(models.Model):
    _name = "hr_service"
    x_direction_id = fields.Many2one('hr.department', 'Structure', required=True)
...
class HrRegistreEmploye(models.Model):
    ...
    name = fields.Many2one('hr.payroll.structure', string='Type employé', required=False)
    company_id = fields.Many2one('res.company', string="Structure", default=lambda self: self.env.user.company_id.id)
    x_exercice_id = fields.Many2one('ref_exercice',
                                    default=lambda self: self.env['ref_exercice'].search([('etat', '=', 1)]))
    ...
    def action_rech(self):
        for val in self:
            domain = []
            if val.name:
                domain.append(('x_type_employe_id', '=', val.name.id))
            if val.x_direction_id:
                domain.append(('x_direction_id', '=', val.x_direction_id.id))
            ...
            for e in employees:
                print("Numero",num)
                if e.x_type_employe_id.code == 'CTRCT':
                    mat = e.matricule_genere
                    cat = e.x_categorie_c_id.name
                    echelle = e.x_echelle_c_id.name
                    echellon = e.x_echellon_c_id.name
                    classe = "-"
                else:
                    mat = e.matricule_genere
                    cat = e.x_categorie_id.name
                    echelle = e.x_echelle_id.name
                    echellon = e.x_echellon_id.name
                    classe = e.x_classees_id.name

                self.env['hr_regemployes_line'].create({
                    'ordre': num,
                    'dep': e.x_direction_id.name,
                    'mat': mat,
                    'employee': e.name,
                    'x_date_naissance': e.x_date_naissance,
                    'x_date_retraite': e.x_date_retraite,
                    'retraite': e.x_nb_annee_retraite.name,
                    'genre': e.genre,
                    'eta_civil': e.situation_marital,
                    'emploi': e.x_emploi_id.name,
                    'fonction': e.x_fonction_id.name,
                    'categorie': cat,
                    'echelle': echelle,
                    'x_classe': classe,
                    'echelon': echellon,
...
class HrRegistreEmployeLine(models.Model):
    ...
    ordre = fields.Char('N°', readonly=False)
    x_date_naissance = fields.Date(string='Date de naissance', required=False,)
    x_date_retraite = fields.Date(string='Date retraite', required=False,)
    employee = fields.Char('Nom et Prénoms', readonly=False)
    mat = fields.Char(string='N° matricule', readonly=False)
    mat_ctrct = fields.Char(string='Mlle Contractuel', readonly=False)
    nom = fields.Char(string='Nom/Prénom(s)', readonly=False)
    categorie = fields.Char(string="Catégorie", readonly=False)
    echelle = fields.Char(string='Echelle', readonly=False)
    x_classe = fields.Char(string='Echelle', readonly=False)
    echelon = fields.Char(string='Echelon', readonly=False)
    genre = fields.Char(string='Genre', readonly=False)
    eta_civil = fields.Char('Etat', readonly=False)
    retraite = fields.Char('Retraite', readonly=False)
    emploi = fields.Char('Emploi', readonly=False)
    fonction = fields.Char('Fonction', readonly=False)
    dep = fields.Char('Département', readonly=False)
    service = fields.Char('Service', readonly=False)
    zone = fields.Char('Zone', readonly=False)
    x_type = fields.Char('Type employé', readonly=False)
...
class HrEmployees(models.Model):
    ...
    def name_get(self):
        ...
        for rec in self:
            if rec.name:
                result.append((rec.id, '%s - %s' % (rec.matricule_genere, rec.name)))
        return result
    ...
    x_date_naissance = fields.Date(string='Date de naissance', required=True,  default=date.today())
    ...
    x_indemnite_residence = fields.Float(string='Indem. de residence', compute="cal_x_indemnite_residence")
    ...
    def create_contrat(self):
        ...
        for val in result:  
            self.env['hr.contract'].create({
                    'name': val.name,
                    'struct_id': val.x_type_employe_id.id,
                    'state': 'open',
                    'wage': 0,
                    'employee_id': val.id})
    ...
    @api.onchange('charge_enfant', 'x_type_employe_id')
    def mnt_alloc(self):
        for vals in self:
            ...
            if vals.x_type_employe_id.code == 'CTRCT':
                ...
                if vals.charge_enfant <= 6:
                    vals.x_allocation_familial = vals.charge_enfant * 2000
                else:
                    vals.x_allocation_familial = 6 * 2000
    ...
    mnt_rappel_solde_indiciaire = fields.Float(string='Solde indiciaire', required=False)
    mnt_rappel_residence = fields.Float(string='Indemn Residence', required=False)
    ...
    # Trop percu
     # Trop percu sur salaire et indemenite
    mnt_percu_salaire_net = fields.Float(string='Rappel Sur Salaire net', required=False, default=0)
    mnt_percu_salaire = fields.Float(string='Salaire de base', required=False)
    mnt_percu_solde_indiciaire = fields.Float(string='Solde indiciaire', required=False)
    mnt_percu_residence = fields.Float(string='Indemn Residence', required=False)
    ...
    mnt_percu_spec_it = fields.Float(string='Indemn.Spec.it', required=False)
    ...
    mnt_percu_iuts = fields.Float(string='Iuts', required=False)
    ...
    #Precompte
    mnt_precompte_avsolde = fields.Float(string='Foner', required=False)
    mnt_foner = fields.Float(string='Foner', required=False)
    x_uab_vie = fields.Float(string='Uab vie', required=False)
    x_sbe = fields.Float(string='Uab vie', required=False)
    x_sonar_vie = fields.Float(string='Uab vie', required=False)
    x_saham_vie = fields.Float(string='Uab vie', required=False)
    x_sunu = fields.Float(string='Uab vie', required=False)
    x_poste_burkina = fields.Float(string='Uab vie', required=False)
    x_cegeci = fields.Float(string='Uab vie', required=False)
    x_sobca = fields.Float(string='Uab vie', required=False)
    gle_vie = fields.Float(string='Uab vie', required=False)

    mnt_pecompte_indiciaire = fields.Float(string='Uab vie', required=False)
    mnt_precompte_allocation = fields.Float(string='Allocation familiale', required=False)
    mnt_precompte_indem_sujet = fields.Float(string='Allocation familiale', required=False)
    mnt_precompte_indem_loge = fields.Float(string='Allocation familiale', required=False)
    mnt_precompte_ish = fields.Float(string='Allocation familiale', required=False)
    mnt_precompte_mena = fields.Float(string='Allocation familiale', required=False)
    mnt_precompte_ensbase = fields.Float(string='Allocation familiale', required=False)
    mnt_precompte_smess = fields.Float(string='Allocation familiale', required=False)
    mnt_precompte_tech = fields.Float(string='Allocation familiale', required=False)
    mnt_precompte_astr = fields.Float(string='Allocation familiale', required=False)
    mnt_precompte_resp = fields.Float(string='Allocation familiale', required=False)
    mnt_precompte_prestage = fields.Float(string='Allocation familiale', required=False)
    mnt_precompte_salbase = fields.Float(string='Allocation familiale', required=False)
    ...
    x_solde_indiciaire_ctrct = fields.Float(string="Salaire de base", required=True, default=0)
    ...
    def calcul_abattement(self):
        for vals in self:
            ...
            if vals.x_type_employe_id.code == 'CTRCT':
                ...
                salaire_base = vals.x_solde_indiciaire
            print("Meda")
            print(salaire_base)
            if vals.x_type_employe_id.code == 'CTRCT':
                if x_cat == '1' or x_cat == '2' or x_cat == '6':
    ...
    def calcul_salaire_brut(self):
        for vals in self:
            ...
            vals.x_salaire_brut = vals.x_remuneration_total - cnss_deductible - vals.x_mnt_carfo - vals.x_allocation_familial
    ...
    def calcul_exoneration_residence(self):
        for vals in self:
            premiere_limite = round((vals.x_salaire_brut * 5) / 100)
            vals.x_indem_residence_exo = min([premiere_limite, vals.x_indemnite_residence, 50000])
    ...
    x_categorie_c_id = fields.Many2one('hr_categorie', string='Catégorie C', required=False)
    x_echelle_c_id = fields.Many2one('hr_echelle', string='Echelle C', required=False)
    x_echellon_c_id = fields.Many2one('hr_echellon', required=False, string='Echelon C')
    ...
    def sal_base(self):
        ...
        if val_class != False and val_echel != False and val_echellon != False and val_cat != False:
            ...
            print('Solde residence', x_indemnite_residence)
            self.x_solde_indiciaire_net = self.x_solde_indiciaire + x_indemnite_residence  # solde indiciaire_net= salaire de base
            self.x_indice = round(res.x_indice)

    @api.depends('x_type_employe_id', 'x_solde_indiciaire')
    def cal_x_indemnite_residence(self):
        for val in self:
            if val.x_type_employe_id.code == 'FCT_MD':
                val.x_indemnite_residence = round((val.x_solde_indiciaire * 10 / 100) + 0.1)
            else:
                val.x_indemnite_residence = 0
    ...
    @api.onchange('x_emploi_id', 'x_zone_id', 'x_echelle_c_id', 'x_categorie_c_id', 'x_categorie_id', 'x_echelle_id')
    def indem_astr(self):
        ...
        if self.x_type_employe_id.code in ('FCT_MD', 'FCT_DETACH'):
            val_cat = self.x_categorie_id.id
            val_echel = self.x_echelle_id.id
    ...
    def indem_loge(self):
        ...
        if val_emploi != False:
            if val_zone != False or val_zone == False and val_echel != False or val_echel == False and val_cat != False or val_cat == False and val_struct != False:
                ...
                #      ('x_categorie_c_id', '=', val_cat), ('company_id', '=', val_struct)])
                res = self.env['hr_paramindemnitelogement'].search(
                    [('x_emploi_id', '=', val_emploi),
                     ('x_categorie_c_id', '=', val_cat),
    ...
    @api.onchange('x_emploi_id', 'x_fonction_id', 'x_zone_id', 'x_echelle_c_id', 'x_categorie_c_id', 'x_categorie_id', 'x_echelle_id')
    ...
    def indem_spec_ish(self):
        ...
        if val_emploi != False:
            if val_zone != False or val_zone == False and val_echel != False or val_echel == False and val_cat != False or val_cat == False and val_struct != False:
                ...
                res = self.env['hr_paramindemnitespecifique_ish'].search(
                    [('x_zone_id', '=', val_zone), ('x_emploi_id', '=', val_emploi), 
                        ('company_id', '=', val_struct)])
    ...
    @api.onchange('x_fonction_id', 'x_zone_id', 'x_categorie_c_id', 'x_categorie_id', 'x_echelle_id')
    ...
    @api.onchange('x_fonction_id', 'x_categorie_c_id', 'x_categorie_id', 'x_echelle_id')
    ...
    code_employee = fields.Char(string="Code employe", required=True, default=0, compute="cal_code_employee")
    ...
    @api.depends('x_type_employe_id')
    def cal_code_employee(self):
        for vals in self:
            vals.code_employee = ''
            if vals.x_type_employe_id:
                vals.code_employee = vals.x_type_employe_id.code

    @api.onchange('fonction')
    def on_change_fonction(self):
        return {'domain': {'x_fonction_id': [('fonction', '=', self.fonction)]}}
...
class HrEmployeesEnfant(models.Model):
    ...
    etat = fields.Selection([
        ...
        ('3', 'Etude'),
        ('4', 'Besion Spécifique'),
    ], 'Status', default='1', index=True, required=True)
